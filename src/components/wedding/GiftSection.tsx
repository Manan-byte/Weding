      ...
      <div className="container max-w-lg mx-auto px-4">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-50px" }} transition={{ duration: 0.8 }} className="text-center mb-10 sm:mb-12">
          ...
          <p className="font-body text-xs sm:text-sm text-muted-foreground max-w-md mx-auto">
            Tanpa mengurangi rasa hormat, bagi yang ingin memberikan doa dalam bentuk tanda kasih dapat melalui rekening berikut:
          </p>
        </motion.div>

        <div className="space-y-4 sm:space-y-5">
          {accounts.map((account, index) => (
            <motion.div key={account.bank} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }} variants={cardVariants} transition={{ duration: 0.6, delay: index * 0.15 }} className="bg-background/95 rounded-lg p-5 sm:p-6 shadow-elegant border border-gold/20 text-center">
